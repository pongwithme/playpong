...
Pong = {
  ...
  Defaults: {
    width:        640*2,   // logical canvas width (browser will scale to physical canvas size - which is controlled by @media css queries)
    height:       480*2,   // logical canvas height (ditto)
    wallWidth:    12*2,
    paddleWidth:  12*2,
    paddleHeight: 60*4,
    paddleSpeed:  .66,     // should be able to cross court vertically   in 2 seconds
    ballSpeed:    3,     // should be able to cross court horizontally in 4 seconds, at starting speed ...
    ballAccel:    8*2,     // ... but accelerate as time passes
    ballRadius:   5*2,
	speedMultiplier: .3,
    sound:        true
  },
  ...
  draw: function(ctx) {
    ctx.fillStyle = Pong.Colors.leftPaddle1;
    this.court.draw(ctx, this.scores[0], this.scores[1]);
    ctx.fillStyle = Pong.Colors.leftPaddle1;
    this.leftPaddle.draw(ctx);
	ctx.fillStyle = Pong.Colors.rightPaddle1;
  ...
  Paddle: {
	...
	// range of gyroscope: -90 to 90
	// range of game: 15 to 825
